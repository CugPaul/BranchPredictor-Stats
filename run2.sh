...
TRACES_SPECTEN="traces/spec2k17_ten"
TRACES_SELECT="620.omnetpp_s"
...
file_trace () 
{
    ...
    for i in $2/*;
    do
        branch_name=$(echo $3 | cut -d'-' -f1)
        OUT=$OUT_BASE/$branch_name  #为每个benchmark创建一个文件夹
        mkdir -p $OUT > /dev/null 2>&1

        trace=$(echo $i | rev | cut -d'/' -f1 | rev) #提取出600.perlbench_s-210B.champsimtrace.xz
        folder_name=$(echo "$trace" | cut -d'-' -f1)  #提取出600.perlbench_s 创建文件夹 
        OUT_2=$OUT/$folder_name  #为每个benchmark创建一个文件夹

        if [[ "$folder_name" == "$TRACES_SELECT" ]]; then
            name=$(echo "$trace" | cut -d'-' -f2| cut -d'.' -f1)  #提取出210B作为结果命名
            if [[ "$LOGGED" == "Y" ]]; then
                echo -n "$1 -warmup_instructions 50000000 -simulation_instructions"
                echo " 200000000 -traces $i > $OUT_2/$name.out 2>>$LOG"
            else
                echo -n "$1 -warmup_instructions 50000000 -simulation_instructions"
                echo " 200000000 -traces $i > $OUT_2/$name.out 2>/dev/null"
            fi
            mkdir -p $OUT_2 > /dev/null 2>&1
        fi
...
run_compile ()  #传入 "./build_champsim.sh hashed_perceptron no vberti no no no no no\
...
while getopts :mfvlrcdhngbp: opt; do  #added -b 一直卡在这 -p后面是更一个参数 我一直把-b放在了最后 导致识别失败
    case "${opt}" in
          v) VERBOSE="Y"
          ...
          m) MULTI="Y"
              ...
              ;;
          b) SPEC_BRANCH="Y"
              echo "Using Ten SPEC2017 Programs Adopted by Branchnet" >> $LOG
              echo "============================================================" >> $LOG
              ;;
          h) print_help;;
     esac
...
if [[ "$BUILD" == "Y" ]]; then
    ...
    cd $BERTI
    run_compile "./build_champsim.sh multiperspective_perceptron8kb_onebranch no vberti no no no no no\
...
for i in $(ls $BERTI/bin/*1core*); do # bin中有的就并行执行
    if [[ "$LOGGED" == "Y" ]]; then
    ...
    aux=$i
    # mkdir $OUT > /dev/null 2>&1
    if [[ "$SPEC_BRANCH" == "N" ]]; then
        file_trace $i $TRACES_SPEC $name >> tmp_par.out #后面几个都是传递给file_trace的参数 这一句将命令追加输出到tmp_par中
    elif [[ "$SPEC_BRANCH" == "Y" ]]; then
        file_trace $i $TRACES_SPECTEN $name >> tmp_par.out
    fi
...
echo -n "Running..."
cat tmp_par.out | xargs -I CMD -P $NUM_THREAD bash -c CMD  #这一句才是真正的跑起来了 调用已经写入到tmp_par.out的命令
echo " ${GREEN}done${NC}"
